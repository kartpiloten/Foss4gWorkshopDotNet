...
using RoverData.Repository; // Data repository abstraction (async APIs)
...
public class ScentPolygonService : IHostedService, IDisposable
{
  ...
  {
    await _dataRepository.InitializeAsync(cancellationToken); // async DB/file init
  ...
  {
    var measurements = await _dataRepository.GetAllAsync();
  ...
  private async void PollForNewMeasurements(object? state)
  {
    ...
    try
    {
      ...
      var newMeasurements = await _dataRepository.GetNewSinceTimestampAsync(_lastKnownTimestampUtc);
      if (!newMeasurements.Any())
      {
        ...
        if (DateTime.UtcNow.Second % 10 == 0)
        {
          StatusUpdate?.Invoke(this, new ScentPolygonStatusEventArgs
          {
            ...
            DataSource = _dataRepository.GetType().Name,
