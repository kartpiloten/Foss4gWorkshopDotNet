...
public class ScentPolygonService : IHostedService, IDisposable
{
 ...
 ScentPolygonConfiguration? configuration = null,
    ...
    // --- Public state (externally used) ---
    public int ActiveRoverCount => _roverProcessors.Count;

    // --- Internal state (not externally used) ---
    internal int Count => _polygons.Count;
    internal ScentPolygonResult? LatestPolygon => _latestPolygon;
    internal int UnifiedVersion => _unifiedVersion;

    internal List<ScentPolygonResult> GetAllPolygons() =>
    _polygons.Values.OrderBy(p => p.Sequence).ToList(); // LINQ: order for deterministic output

    /// <summary>
    /// Gets the unified polygon for a specific rover (internal - not used externally)
    /// </summary>
    internal RoverUnifiedPolygon? GetRoverUnifiedPolygon(Guid roverId)
    {
    ...
    /// <summary>
    /// Non-cached unified polygon (calls calculator each time) - internal, not used externally
    /// </summary>
    internal UnifiedScentPolygon? GetUnifiedScentPolygon()
    {
    ...
    /// <summary>
    /// Generates a scent polygon for a specific measurement (internal - only used within service).
    /// </summary>
    internal ScentPolygonResult GeneratePolygonForMeasurement(RoverMeasurement measurement)
    {
