...
public static class ScentPolygonCalculator
{
        ...
        GeometryFactory? geometryFactory = null)
    {
        ...
        var bufferRadiusDegrees = (config.OmnidirectionalRadiusMeters > 0 ? config.OmnidirectionalRadiusMeters : 0) / metersPerDegLat;
        Geometry combined = fan;
        if (bufferRadiusDegrees > 0)
        {
            var smallBuffer = dogPoint.Buffer(bufferRadiusDegrees);
            combined = fan.Union(smallBuffer);
        }

        // Normalize to Polygon (largest part if multipolygon)
        Polygon finalPolygon;
        if (combined is Polygon single)
        {
            finalPolygon = single;
        }
        else if (combined is MultiPolygon multi)
        {
            finalPolygon = multi.Geometries.OfType<Polygon>().OrderByDescending(p => p.Area).First();
        }
        else
        {
            finalPolygon = fan; // fallback
        }
