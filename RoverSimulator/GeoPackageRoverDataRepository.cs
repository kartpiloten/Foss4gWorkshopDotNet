...
public class GeoPackageRoverDataRepository : RoverDataRepositoryBase
{
 ...
 public GeoPackageRoverDataRepository(string connectionString, string sessionTableName) 
 ...
 _folderPath = Environment.CurrentDirectory;
        ...
        _dbPath = Path.Combine(_folderPath, $"session_{sessionTableName}.gpkg");
 }

 /// <summary>
 /// Initializes the repository, creating the GeoPackage file and measurements layer.
 /// </summary>
...
["rover_name"] = "TEXT NOT NULL",
 ...
 /// <summary>
 /// Resets the database, deleting the existing GeoPackage file if it exists.
 /// </summary>
 ...
 File.Delete(_dbPath);
             Console.WriteLine($"Deleted existing GeoPackage for session '{_sessionTableName}': {Path.GetFileName(_dbPath)}");
 }
 ...
 /// <summary>
 /// Inserts a single measurement into the GeoPackage.
 /// </summary>
...
if (processName.Contains("qgis") || 
 ...
 if (File.Exists(_dbPath))
 {
  ...
  {
     Console.WriteLine("File is available and can be used.");
            }
      }
        else
     {
 ...
 var fileInfo = new FileInfo(_dbPath);
            Console.WriteLine($"Final GeoPackage stats for session '{_sessionTableName}':");
