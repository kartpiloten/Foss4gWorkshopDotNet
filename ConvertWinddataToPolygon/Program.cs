...
try
{
    // Create rover data repository using the new architecture
    Console.WriteLine("Setting up rover data repository...");
    
    IRoverDataRepository? repository = null;
    
    // Try PostgreSQL first, then fallback to GeoPackage
    try
    {
    Console.WriteLine("Attempting to connect to PostgreSQL database...");
    var postgresConnectionString = ConverterConfiguration.PostgresConnectionString;
    
    if (!string.IsNullOrEmpty(postgresConnectionString))
    {
        var opts = Options.Create(new PostgresRepositoryOptions
        {
            ConnectionString = postgresConnectionString,
            SessionName = "default"
        });
        repository = new PostgresRoverDataRepository(opts);
    ...
    var folderPath = Path.GetDirectoryName(ConverterConfiguration.GeoPackageFilePath) ?? "/tmp/Rover1";
    var opts = Options.Create(new GeoPackageRepositoryOptions
    {
        FolderPath = folderPath,
        SessionName = "default"
    });
    repository = new GeoPackageRoverDataRepository(opts);
    await repository.InitializeAsync();
    ...
    var measurementCount = await repository.GetCountAsync();
    ...
    Console.WriteLine($"\nData source used: {(repository is PostgresRoverDataRepository ? "PostgreSQL database" : "GeoPackage file")}");
...
public static class ConverterConfiguration
{
    ...
    Configuration.GetValue<string>("DatabaseConfiguration:ConnectionString");
}
